...
void CHard::SaveData()
{
	...
	if (alltriggers > 1)
	{
		...
		if (evt_offset == -1)
		{
			...
			//GetSystemTime(&st);
			GetLocalTime(&lt);
			CString filename; filename.Format("%d_%d_%d_%d_%d.data", HantekId+1, lt.wYear, lt.wMonth, lt.wDay, lt.wHour);
			pulselastfile = fopen(filename, "a");
			fprintf(pulselastfile, "%5.1f %5.1f %5.1f %5.1f\n", -99.0, -99.0,  -99.0, -99.0);
			//fprintf(pulselastfile, "%d %d %d\n", lt.wMinute, lt.wSecond, lt.wMilliseconds);
...
void CHard::DoChangeTrigger()
{
	...
	default = (default + 1) % 3; //if (default == 3)default = 0;
	Init();
...
void CHard::ReadSCANData()
{
	...
	CString str;
	str.Format(_T("±¾´ÎSCan»ñÈ¡µÄ³¤¶È%d\t ÏÖÔÚ×Ü¹²³¤¶ÈÎª%d\n"), nCurReadLen, nCurSCANLen);
